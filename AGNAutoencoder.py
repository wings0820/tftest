...
class AdditiveGaussianNoiseAutoencoder(object):
         ...
         def partial_fit(self,X):
             cost,opt=self.sess.run((self.cost,self.optimizer),feed_dict={self.x:X,self.scale:self.
             training_scale})
             return cost
...
for epochs in range(training_epochs):
    ...
    if epochs % display_step==0:
        print("Epoch:",'%04d'%(epcho+1),"cost = ","{:.9f}".format(avg_cost))
